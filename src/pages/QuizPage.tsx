...
const QuizPage: React.FC = () => {
    ...
    return (
        <div className='quiz-page'>
            <h1>QuizPage</h1>
            { currentQuestionIndex > 0 && <Button label="Previous Question" onClick={handlePreviousQuestion}/>}
            { currentQuestionIndex < questions.length && <Button label="Next Question" onClick={handleNextQuestion}/>}
            { <Question question={questions[currentQuestionIndex]} /> }
            { currentQuestionIndex === questions.length && <Button label="Done" onClick={()=> console.log("DONE!")}/>}
        </div>
    )
}
